...
getAvailable = (hero) ->
    skills = {}
    for key, value of skillList when value.requirements? and value.requirements.level <= hero.level and not hero.skills[key]
        skills[key] = value
    return skills

getCoreSkills = (heroClass) ->
    skills = {}
    for key, value of skillList when value.core? and value.core.has heroClass
        skills[key] = value
    return skills
...
isAvailable = (hero, skillKey) ->
    skill = skillList[skillKey]
    if hero.level < skill.requirements.level then return false

    if skill.requirements.melee? and skill.requirements.melee > hero.masteries.melee.level
        err ' -- melee mastery requirement not reached (you have: ' + hero.masteries.melee.level + ', required: ' + skill.requirements.melee + ')'
        return false
    if skill.requirements.ranged? and skill.requirements.ranged > hero.masteries.ranged.level
        err ' -- ranged mastery requirement not reached (you have: ' + hero.masteries.ranged.level + ', required: ' + skill.requirements.ranged + ')'
        return false
    if skill.requirements.magic? and skill.requirements.magic > hero.masteries.magic.level
        err ' -- magic mastery requirement not reached (you have: ' + hero.masteries.magic.level + ', required: ' + skill.requirements.magic + ')'
        return false
